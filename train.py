...
def getParameters():
    ...
    parser.add_argument('-c', '--cuda', type=str, default="0",
                                help="GPU devices to use, e.g., '0' for single GPU, '0,1,2,3' for multiple GPUs")
...
def main():
    ...
    if config.cuda != "-1":  # 不使用CPU
        ...
        print(f"Using GPU devices: {config.cuda}")
    ...
    package     = __import__(moduleName, fromlist=['Trainer'])
