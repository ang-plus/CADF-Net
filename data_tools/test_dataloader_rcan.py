...
class TestDataset:
    def __init__(   self,
        ...
        # 移除Normalize，因为训练时在DataPrefetcher中手动处理
    ...
    def __call__(self):
        ...
        # Debug: print type
        # print(type(hr), type(lr))
        if not isinstance(hr, torch.Tensor):
            hr = torch.from_numpy(np.array(hr)).permute(2,0,1).float()
        if not isinstance(lr, torch.Tensor):
            lr = torch.from_numpy(np.array(lr)).permute(2,0,1).float()
