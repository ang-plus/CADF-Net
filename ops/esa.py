...
class ESA(nn.Module):
    ...
    def __init__(self, esa_channels, n_feats, conv=nn.Conv2d):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(n_feats, n_feats, kernel_size=1, groups=n_feats, bias=False),  # 深度卷积
            nn.Conv2d(n_feats, f, kernel_size=1, bias=False)  # 点卷积
        )
...
class AdaConvGuidedFilter(nn.Module):
    ...
    def __init__(self, esa_channels, n_feats, conv=nn.Conv2d, kernel_expand=1,bias = True):
        ...
        self.gap = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Conv2d(in_channels=f, out_channels=f, kernel_size=1, padding=0, stride=1,
                      groups=1, bias=True)
