...
from ops.CDSBlock import CDSBlock
# helpers
...
class MBConvResidual(nn.Module):
    def __init__(self, fn, dropout = 0.):
        super().__init__()
        self.fn = fn
        self.dropsample = Dropsample(int(dropout))
...
class CDSGroup(nn.Module):
    def __init__(self, channel_num=64, bias = True, ffn_bias=True, window_size=8, with_pe=False, dropout=0.0):
        super(CDSGroup, self).__init__()
        self.cds_block = CDSBlock(channel_num)
    def forward(self, x):
        return self.cds_block(x)