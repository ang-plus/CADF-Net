...
class DataPrefetcher():
    def __init__(self, loader):
        ...
        # self.stream = torch.cuda.Stream()  # Remove stream usage
    ...
    def __preload__(self):
        try:
            ...
            self.hr, self.lr = next(self.dataiter)
        # Remove stream context, just move to cuda
...
class DIV2K_Flickr_Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return hr_img.float(), lr_img.float() # 返回float tensor
...
def GetLoader(  dataset_roots,
    ...
    # 移除ToTensor()和Normalize()，因为数据已经在预处理中转换为float并归一化了
